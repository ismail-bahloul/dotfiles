...
The desktop (terminal + system monitor + prompt) takes the colors of the
...
and the p10k prompt all read that same file → total consistency. The parsing
...
`material-you-refresh`** (kitty + btop + prompt cache) → kitty reloads
(`pkill -USR1`).
